...
(function () {
  ...
  QRCode = function (canvasId, vOption) {
    ...
    if (this._htOption.text && this.canvasId) {
      this.makeCode(this._htOption.text, this._htOption.callback);
    }
  };
  ...
  QRCode.prototype.makeImage = function (callback) {
    ...
    if (_htOption.image && _htOption.image != '') {
      const imgW = _htOption.width / 4;
      const imgH = _htOption.height / 4;
      const imgX = (_htOption.width - imgW) / 2;
      const imgY = (_htOption.height - imgH) / 2;
      _oContext.drawImage(_htOption.image, imgX, imgY, imgW, imgH)
    }
  ...
  // 保存为图片，将临时路径传给回调
  QRCode.prototype.exportImage = function (callback, failCb) {
    if (!callback) {
      return
    }
    wx.canvasToTempFilePath({
      ...
      destWidth: this._htOption.width * 8,
      destHeight: this._htOption.height * 8,
